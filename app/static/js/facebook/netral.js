document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateTable(data) {
        if (data.status === 'success') {
            ...
            // Filter to only show netral sentiment
            const netralComments = commentsArray.filter(comment => comment.sentimen === 'Netral');

            // Clear tbody before filling with new data
            tbody.innerHTML = '';

            if (netralComments.length > 0) {
                // Loop through each comment in data
                netralComments.forEach(comment => {
                ...
                // Prepare top 5 netral comments for notification dropdown
                const netralNotificationComments = commentsArray
                ...
                netralNotificationComments.forEach(comment => {
                ...
                console.error('Error processing comments: Empty netralComments array');
            }
        } else {
